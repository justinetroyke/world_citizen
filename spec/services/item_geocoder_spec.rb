...
describe "updates business location to lat/long" do
  xit 'it adds business lat/long' do
    ...
    ItemGeocoderService.new().update

    expect(item.business.longitude).to eq('-105.0742362')
    expect(item.business.latitude).to eq('39.8881213')
    # expect(Item.find(item.id).business_lat).to eq('39.8881213')
    # expect(Item.find(item.id).business_lng).to eq('-105.0742362')
  end

  xit 'it adds org lat/long to item' do
    fb = Category.create!(name: 'F&B')
